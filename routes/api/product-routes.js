...
// get all products
  ...
  router.get('/:id', async (req, res) => {
    ...
    // be sure to include its associated Category and Tag data
    try {
      const getProductID = await Product.findByPk(req.params.id, {include: [{ model: Category }, { model: Tag }]});
      if (!getProductID) {
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!deleteProduct) {res.status(404).json({ message: 'No product found with this ID.' });
      return;
    }
